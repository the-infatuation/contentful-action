export declare const GITHUB_WORKSPACE: string | undefined, GITHUB_REF_NAME: string | undefined, LOG_LEVEL: string | undefined, SPACE_ID: string | undefined, MANAGEMENT_API_KEY: string | undefined, INPUT_MIGRATIONS_DIR: string | undefined, INPUT_DELETE_FEATURE: string | undefined, INPUT_SET_ALIAS: string | undefined, INPUT_FEATURE_PATTERN: string | undefined, INPUT_MASTER_PATTERN: string | undefined, INPUT_VERSION_CONTENT_TYPE: string | undefined, INPUT_VERSION_FIELD: string | undefined, INPUT_CONTENTFUL_ALIAS: string | undefined, INPUT_DELAY: string | undefined, INPUT_MAX_NUMBER_OF_TRIES: string | undefined, INPUT_CREATE_CDA_TOKEN: string | undefined, INPUT_DEFAULT_BRANCH_NAME: string | undefined, INPUT_ACTIONS: string | undefined;
export declare const DEFAULT_MIGRATIONS_DIR = "migrations";
export declare const DEFAULT_MASTER_PATTERN = "master-[YYYY]-[MM]-[DD]-[mm][ss]";
export declare const DEFAULT_FEATURE_PATTERN = "GH-[branch]";
export declare const DEFAULT_VERSION_CONTENT_TYPE = "versionTracking";
export declare const DEFAULT_VERSION_FIELD = "version";
export declare const DEFAULT_DELETE_FEATURE = false;
export declare const DEFAULT_SET_ALIAS = false;
export declare const DEFAULT_CONTENTFUL_ALIAS = "master";
export declare const DEFAULT_DELAY = 3000;
export declare const DEFAULT_MAX_NUMBER_OF_TRIES = 10;
export declare const DEFAULT_CREATE_CDA_TOKEN = true;
export declare const DELETE_FEATURE: boolean;
export declare const CREATE_CDA_TOKEN: boolean;
export declare const SET_ALIAS: boolean;
export declare const DEFAULT_ACTIONS: (string | false)[];
export declare const VERSION_CONTENT_TYPE: string;
export declare const FEATURE_PATTERN: string;
export declare const MASTER_PATTERN: string;
export declare const VERSION_FIELD: string;
export declare const MIGRATIONS_DIR: string;
export declare const CONTENTFUL_ALIAS: string;
export declare const DELAY: number;
export declare const MAX_NUMBER_OF_TRIES: number;
export declare const DEFAULT_BRANCH_NAME: string | null;
export declare const ACTIONS: (string | false)[];
